...
here: <https://dashpay.atlassian.net/wiki/spaces/DOC/pages/124092516/Dash+v12.2+-+2017-11-07>
...
* If running `dashd` as a systemd service, then
...
Scenario: Running dash from my home directory...
```
...
### [2] Enable dashcore repository

As of Dash Core version 12.1, we moved to the Fedora COPR build system for
delivery of packaging. To enable the repository, if you haven't already, simply
...
### [4] Remove cache data (shouldn't really need to do this)

Assuming your data directory is in `~/.dashcore`, do this...
```
cd ~
file .dashcore # Does .dashcore exist?
...
Assuming your data directory is in `/var/lib/dashcore`, you were running `dashd` as a
...
# Become root user
sudo su -
# Manually start Dash 12.2 and reindex...
...
```
# Start and enable Dash Core as a systemd managed service...

...
# Note on the above change
#t0dd: our RPM SPEC file BuildRequires gmp-devel which provides gmp.h
#t0dd: our RPM SPEC file BuildRequires cmake as well
#t0dd: BUT it seems to want a static build of gmp, which we can't provide
...
#t0dd## Original version (only the cmake call changes):
#todd#define $(package)_config_cmds
#todd#  export CC="$($(package)_cc)" && \
#todd#  export CXX="$($(package)_cxx)" && \
#todd#  export CFLAGS="$($(package)_cflags) $($(package)_cppflags)" && \
#todd#  export CXXFLAGS="$($(package)_cxxflags) $($(package)_cppflags)" && \
#todd#  export LDFLAGS="$($(package)_ldflags)" && \
#todd#  $(host_prefix)/bin/cmake ../ $($(package)_config_opts)
#todd#endef

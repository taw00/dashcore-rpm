...
# If systemd-managed...
LF="sudo -u dashcore dash-cli -conf=/etc/dashcore/dash.conf -datadir=/var/lib/dashcore masternode list full"
...
percent_not_enabled_old=$(printf '%6.2f' `python -c "print round($total_not_enabled_old.0 / $total_not_enabled * 100, 2)"`)
...
percent_not_enabled_new=$(printf '%6.2f' `python -c "print round($total_not_enabled_new.0 / $total_not_enabled * 100, 2)"`)
